...
let ``should parse lock file``() = 
    ...
    packages.[4].Source |> shouldEqual (NugetSources.NugetSource.GetRemoteFeed "http://nuget.org/api/v3")
    packages.[4].Name |> shouldEqual "Rx-Core"
    packages.[4].Version |> shouldEqual (SemVer.Parse "2.1")

    packages.[5].Source |> shouldEqual (NugetSources.NugetSource.GetRemoteFeed "http://nuget.org/api/v3")
